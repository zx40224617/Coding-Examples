...
public class Queue<T extends Comparable<T>> {
     ...
     * to determine if a specific element is in the Queue
    ...
    public boolean binarySearch(T value) throws IllegalStateException { 
    	if (!this.isSorted()) {
    		throw new IllegalStateException("binarySearch(): The queue is not sorted");
    	} else {
    		if (binarySearch(0, this.size - 1, value) != -1) {
    			return true;
    		} else {
    			return false;
    		}
